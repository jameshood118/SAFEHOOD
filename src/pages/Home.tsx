...
export const HomeOS = () => {
  ...
  const { data: intelList, isLoading, isError, error } = useQuery({
  ...
  const dumpIntelMutation = useMutation({
    mutationFn: async () => {
      ...
      if (dropzoneText.trim().startsWith('{') || dropzoneText.trim().startsWith('<')) type = 'CODE_SNIPPET';
  ...
  const purgeIntelMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('base_camp_intel').update({ is_deleted: true }).eq('id', id);
      if (error) throw error;
    },
  ...
  const getTypeIcon = (type: string) => {
    switch(type) {
      ...
      case 'CODE_SNIPPET': return faCode;
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2, pt: 2, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <Button
            ...
            sx={{
              bgcolor: '#FF9800', color: '#000', fontWeight: 800, fontFamily: 'monospace', borderRadius: 0,
      ...
      {/* --- STATE: LOADING / ERROR --- */}
      {isLoading && <CircularProgress sx={{ color: '#FF9800', display: 'block', mx: 'auto', mt: 4 }} />}
      ...
      {!isLoading && !isError && (
        <Grid container spacing={3}>
          {intelList?.length === 0 ? (
             <Grid size={{ xs: 12 }}>
                <Typography variant="body1" sx={{ color: 'text.disabled', textAlign: 'center', mt: 4, fontFamily: 'monospace' }}>
              ...
              <Grid size={{ xs: 12, md: 6, lg: 4 }} key={intel.id}>
                <Card sx={{
                  ...
                  <CardContent sx={{ p: 3, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                      <Chip
                        ...
                        size="small"
                        sx={{ bgcolor: 'rgba(255, 152, 0, 0.1)', color: '#FF9800', fontFamily: 'monospace', borderRadius: 0 }}
                      />
                      <Typography variant="caption" sx={{ color: 'text.disabled', fontFamily: 'monospace' }}>
                    ...
                    <Box sx={{
                      ...
                      <Typography variant="body2" sx={{ color: 'primary.contrastText', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
                        {intel.raw_content}
                      </Typography>
                    </Box>
                  ...
                  <Box sx={{ display: 'flex', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    <Button
                      fullWidth
                      sx={{ borderRadius: 0, color: '#00E5FF', py: 1.5, fontFamily: 'monospace', borderRight: '1px solid rgba(255,255,255,0.05)' }}
                    ...
                    <IconButton
                      ...
                      sx={{ borderRadius: 0, color: '#F44336', width: '60px', '&:hover': { bgcolor: 'rgba(244, 67, 54, 0.1)' } }}
                    >
